...
if (!class_exists("rsssl_licensing")) {
	class rsssl_licensing {
		...
		public function get_license_status(string $action = 'check_license', bool $clear_cache = false ): string {
			...
			$status = rsssl_get_cache('pro_license_status', 'license');
			...
			if ( !$status || get_site_option('rsssl_pro_license_activation_limit') === false ){
				...
				//set default
				rsssl_set_cache('pro_license_status', 'empty', 'license', $transient_expiration);
				...
				if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
					...
					rsssl_set_cache('pro_license_status', 'error', 'license', $transient_expiration);
					update_site_option('rsssl_license_attempts', $attempts );
				} else {
					update_site_option('rsssl_license_attempts', 0 );
				...
				rsssl_set_cache('pro_license_status', $status, 'license', $transient_expiration);
			}
		...
		public function rest_api_license( array $response, string $action, $data): array {
			...
			if ( $action==='activate_license' ) {
					...
					rsssl_delete_cache('pro_license_status', 'license');
					$this->get_license_status('activate_license', true );
					return $this->license_notices($response, $action, $data);
				}
