...
use Exception;
use RSSSL\Pro\Security\WordPress\Two_Fa\Controllers\Rsssl_Totp_Controller;
...
//		add_action( 'rest_api_init', array( $this, 'register_rest_routes' ) );
    ...
    {
        new Rsssl_Totp_Controller($namespace, $version, $featureVersion);
    }
	 ...
	 *
	 * @param int $user_id  WP_User object representing the user for whom to setup TOTP.
	...
	public static function setup_totp( WP_User $user, string $key, string $code ): bool {
		...
		if ( ! self::get_authcode_valid_ticktime( $key, $code ) ) {
	...
	public function validate_authentication(WP_User $user ): bool {
		...
		if ( $code_backup && Rsssl_Two_Factor_Backup_Codes::validate_code( $user->ID, $code_backup ) ) {
			return true;
		}
	 ...
	 */
	public function validate_code_for_user( WP_User $user, string $code ): bool
    {
		$totp_key = $this->get_user_totp_key( $user->ID );
...
//            error_log('failed on repeated code');
//			return false;
//		}
	...
	public function authentication_page( WP_User $user ) {
		...
		<p>
			...
			<input type="tel" inputmode="numeric" autocomplete="one-time-code" autocorrect="off" autocapitalize="none" spellcheck="false" name="authcode" id="authcode" class="input authcode rsssl-authcode" value="" size="20" pattern="[0-9 ]*" placeholder="123 456" data-digits="<?php echo esc_attr( self::DEFAULT_DIGIT_COUNT ); ?>" />
		</p>
	...
	public static function base32_decode( string $base32_string ): string {
		...
		for ( $i = 0; $i < $l; $i++ ) {
			...
			$n += strpos( self::$base_32_chars, $base32_string[ $i ] );    // Add value into buffer.
	...
	public static function is_optional( WP_User $user ): bool {
		...
		// Check if any of the user's roles are in the fallback roles list.
		return ! empty( array_intersect( $user_roles, $fallback ) );
	}
	...
	public static function get_selection_option( WP_User $user, bool $checked = false ): void {
		...
		$user_roles      = ! empty( $user->roles ) && is_array( $user->roles ) ? $user->roles : [];
		$forced_roles    = (array) rsssl_get_option( 'two_fa_forced_roles_totp' );
	...
	public static function display_onboarding_step_totp( WP_User $user ): void {
		...
		wp_add_inline_script('rsssl-profile-settings', $totp_data_js, 'after');
		wp_add_inline_script('rsssl-frontend-settings', $totp_data_js, 'after');
	...
	public static function is_enabled( WP_User $user ): bool {
        ...
        if ( is_multisite() ) {
            $strict_roles = Rsssl_Two_Factor_Settings::get_strictest_role_across_sites( $user->ID, array( 'totp' ) );
            if ( is_string( $strict_roles ) && $strict_roles !== '' ) {
                $user_roles = array( $strict_roles );
            } elseif ( is_array( $strict_roles ) ) {
