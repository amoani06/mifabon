...
class rsssl_headers {
	...
	public function get_csp_rules(): string {
		...
		// If frame ancestors are set, add them to the enforced rule string when the site is not in learning mode.
		$frame_ancestors_value = rsssl_get_option( 'csp_frame_ancestors' );
		if ( ! empty( $frame_ancestors_value ) && $frame_ancestors_value !== 'disabled' && ! $this->header_is_set_by_thirdparty( 'csp_frame_ancestors' ) ) {
			...
			if ( in_array( $frame_ancestors_value, [ 'none', 'self' ], true ) ) {
				$sources = $frame_ancestors_value === 'self' ? trim( "'self' $urls" ) : $urls;
				$sources = empty( $sources ) ? "'none'" : $sources;
				$frame_ancestors_rules = "frame-ancestors $sources; ";
			}
		...
		// If learning mode is true, return whichever of the enforced and report-only rule strings is non-empty.
		if ( rsssl_get_option( 'csp_status' ) === 'learning_mode' ) {
			$headers = [];
			$enforced_rules = $upgrade_insecure_request_rules . $frame_ancestors_rules;
			if ( ! empty( $enforced_rules ) ) {
				$headers[] = $this->wrap_header( 'Content-Security-Policy', $enforced_rules );
			}
			if ( ! empty( $rules ) ) {
				$headers[] = $this->wrap_header( 'Content-Security-Policy-Report-Only', $rules );
			}
	...
	public function insert_security_headers( string $rules, bool $include_csp = true ): string {
		// Skip generating security header rules if wp-content is not writable
	...
	public function wrap_header( string $header, string $rules ): string {
		$break = "\n";
		return 'header(' . var_export($header . ": " . $rules, true) . ');' . $break;
	}
	...
	public function is_header_check_running(): bool {
		...
		$headers = rsssl_get_cache('detected_headers', 'admin', false, get_current_blog_id());
	...
	public function rest_api_clear_cache( $data): array {
		...
		if ($cache_id==='detected_headers') {
			rsssl_delete_cache('detected_headers', 'admin', get_current_blog_id());
	...
	public function clear_headers_check_cache($field_id, $field_value, $prev_value, $field_type) {
		...
		// No header tile, but changes must still bust the detected_headers cache.
		$header_ids[] = 'csp_frame_ancestors_urls';
	...
	public function reload_headers_check_cache(){
		...
		if ( !rsssl_get_cache('detected_headers', 'admin', false, get_current_blog_id()) ){
			$this->get_detected_security_headers();
		}
	}
	...
	public function get_detected_security_headers( $type = false ) {
		$nonce = get_site_option( "rsssl_header_detection_nonce" );
		$can_manage = rsssl_user_can_manage();
		$blog_id = get_current_blog_id();
		if ( ! in_array( $type, [ 'thirdparty', 'all' ] ) ) {
			$type = 'thirdparty';
		}

		$headers = $can_manage ? rsssl_get_cache('detected_headers', 'admin', false, $blog_id) : false;
		if ( ! $headers ) {
			// Set a default
			$headers = [
			...
			if ( $can_manage ) {
				rsssl_set_cache('detected_headers', $headers, 'admin', DAY_IN_SECONDS, $blog_id);
			}
