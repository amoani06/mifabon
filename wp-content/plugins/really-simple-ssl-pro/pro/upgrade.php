...
//	}
    ...
    if ( $prev_version && version_compare( $prev_version, '9.5.11', '<=' ) ) {
        ...
        if ($wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $wpdb->esc_like($table)))) {
            $result = $wpdb->query(
                $wpdb->prepare('ALTER TABLE %i MODIFY `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT', $table)
            );

            if (false === $result && defined('WP_DEBUG') && WP_DEBUG && $wpdb->last_error) {
                error_log('RSSSL: Failed to upgrade event log table id column: ' . $wpdb->last_error);
            }
        }
    }
