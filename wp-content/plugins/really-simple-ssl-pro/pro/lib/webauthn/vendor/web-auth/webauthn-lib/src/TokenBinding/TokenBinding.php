 ...
 */
namespace RSSProVendor\Webauthn\TokenBinding;
...
class TokenBinding
{
    ...
    public static function createFormArray(array $json): self
    {
        ...
        $status = $json['status'];
        Assertion::inArray($status, self::getSupportedStatus(), sprintf('The member "status" is invalid. Supported values are: %s', implode(', ', self::getSupportedStatus())));
