...
use RSSProVendor\Cose\Algorithm\Manager;
use RSSProVendor\Cose\Algorithm\Signature\Signature;
...
use RSSProVendor\Webauthn\Counter\ThrowExceptionIfInvalid;
use RSSProVendor\Webauthn\TokenBinding\TokenBindingHandler;
use RSSProVendor\Webauthn\Util\CoseSignatureFixer;
class AuthenticatorAssertionResponseValidator
{
    ...
    {
        try {
            $this->logger->info('Checking the authenticator assertion response', ['credentialId' => $credentialId, 'authenticatorAssertionResponse' => $authenticatorAssertionResponse, 'publicKeyCredentialRequestOptions' => $publicKeyCredentialRequestOptions, 'host' => $request->getUri()->getHost(), 'userHandle' => $userHandle]);
            /** @see 7.2.1 */
            ...
            /** @see 7.2.11 */
            $rpIdHash = hash('sha256', $expectedRpIdHash, \true);
            ...
            if (null !== $extensionsClientOutputs) {
                $this->extensionOutputCheckerHandler->check($publicKeyCredentialRequestOptions->getExtensions(), $extensionsClientOutputs);
            }
            /** @see 7.2.15 */
            $getClientDataJSONHash = hash('sha256', $authenticatorAssertionResponse->getClientDataJSON()->getRawData(), \true);
            /** @see 7.2.16 */
