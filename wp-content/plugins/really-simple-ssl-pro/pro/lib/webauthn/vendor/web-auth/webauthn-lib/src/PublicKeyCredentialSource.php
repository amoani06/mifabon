...
use RSSProVendor\Ramsey\Uuid\UuidInterface;
use function RSSProVendor\Safe\base64_decode;
use function RSSProVendor\Safe\sprintf;
use Throwable;
use RSSProVendor\Webauthn\TrustPath\TrustPath;
use RSSProVendor\Webauthn\TrustPath\TrustPathLoader;
/**
    ...
    public static function createFromArray(array $data): self
    {
        ...
        switch (\true) {
            case 36 === mb_strlen($data['aaguid'], '8bit'):
                ...
                break;
            default:
                // Kept for compatibility with old format
                $decoded = base64_decode($data['aaguid'], \true);
                $uuid = Uuid::fromBytes($decoded);
        }
        try {
            return new self(Base64Url::decode($data['publicKeyCredentialId']), $data['type'], $data['transports'], $data['attestationType'], TrustPathLoader::loadTrustPath($data['trustPath']), $uuid, Base64Url::decode($data['credentialPublicKey']), Base64Url::decode($data['userHandle']), $data['counter'], $data['otherUI'] ?? null);
