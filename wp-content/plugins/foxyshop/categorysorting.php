...
function foxyshop_category_update_order() {
	...
	$returned_ids = explode(",", sanitize_text_field($_POST['foxyshop_category_order_value']));
...
function foxyshop_category_sort() {
	...
	if ($success) echo wp_kses_post( $success);
	...
	if ($product_categories) {
		...
		if (count($subcats) > 0 || $categoryID > 0) {
			...
			echo ( foxy_wp_kses_html(implode("\n",$subcats), ['option']));
			echo ( '</select>'."\n");
	...
	if ($product_categories) {
		...
		echo wp_kses_post( '<p><em>' . __('No Sub-Categories Found.', 'foxyshop') . '</em></p>');
	}
	?>
	</div>
<?php
