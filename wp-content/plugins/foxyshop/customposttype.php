...
function foxyshop_manage_custom_columns($column_name, $id) {
	...
	case 'productimage':
		...
		if (!$src) $src = FOXYSHOP_PLUGIN_URL."images/no-photo.png";
		echo ($src != 'none' ? ('<a href="post.php?post=' . esc_attr($id) . '&amp;action=edit"><img src="' . esc_url($src) . '" alt="" /></a>') : '&nbsp;');
		break;
	case 'productcode':
		$productcode = get_post_meta($id, "_code", true);
		echo esc_attr($productcode ? $productcode : '(' . $id . ')');
		break;
	case 'price':
...
function foxyshop_product_meta_init() {
	...
	function foxyshop_inline_customposttype_init_js() {
		global $post, $foxyshop_settings;
	...
	add_action( 'admin_print_scripts', 'foxyshop_inline_customposttype_init_js' );
...
function foxyshop_product_details_setup() {
	...
	<div id="foxyshop_price" class="foxyshop_field_control">
		...
		<input type="text" name="_price" id="_price" value="<?php echo esc_attr($_price); ?>" onblur="foxyshop_check_number(this);" style="width: 90px; float: left;" />
	...
	<div id="foxyshop_weight" class="foxyshop_field_control">
		...
		<input type="text" name="_weight1" id="_weight1" value="<?php echo esc_attr($_weight[0]); ?>"<?php if ($disable_weight_checked) echo ' disabled="disabled"'; ?> />
	...
	<div id="foxyshop_quantity" class="foxyshop_field_control">
		...
		<input type="text" name="_quantity_min" id="_quantity_min" value="<?php echo esc_attr($_quantity_min); ?>" title="<?php _e('Minimum Quantity', 'foxyshop'); ?>" style="width: 33px; float: left;" onblur="foxyshop_check_number_single(this);"<?php if ($_quantity_hide) echo ' disabled="disabled"'; ?> />
		...
		<input type="text" name="_quantity_max" id="_quantity_max" value="<?php echo esc_attr($_quantity_max); ?>" title="<?php _e('Maximum Quantity', 'foxyshop'); ?>" style="width: 33px; float: left;" onblur="foxyshop_check_number_single(this);"<?php if ($_quantity_hide) echo ' disabled="disabled"'; ?> />
		<span id="quantity_max_label" style="float: left; margin: 6px 0 0 1px; width: 26px;" class="iconsprite <?php echo esc_attr($_quantity_max ? "up_color" : "up_gray"); ?>"></span>
		...
		<label id="quantity_hide_label" for="_quantity_hide" style="float: left; margin: 6px 0 0 2px; width: 16px;" title="<?php _e('Hide Quantity Box', 'foxyshop'); ?>" class="iconsprite <?php echo esc_attr($_quantity_hide ? "hide_color" : "hide_gray"); ?>"></label>
		<div style="clear:both"></div>
	</div>
	...
	<div id="foxyshop_add_to_cart_link" class="foxyshop_field_control">
		...
		<input type="text" name="add_to_cart_link" id="add_to_cart_link" value="<?php echo esc_attr(foxyshop_product_link("", 1)); ?>" onclick="this.select();" readonly="readonly" />
	</div>
	<?php } ?>
	...
	echo '<input type="hidden" name="menu_order" value="' . esc_attr($post->menu_order == 0 && $post->post_status == "auto-draft" ? $post->ID : $post->menu_order) . '" />';
}
...
function foxyshop_product_pricing_setup() {
	...
	<div class="foxyshop_field_control">
		...
		<input type="text" name="_saleprice" id="_saleprice" value="<?php echo esc_attr($_saleprice); ?>" style="width: 87px; float: left;" />
		<span style="float: left; margin: 9px 0 0 5px;">0.00</span>
	</div>
	<div class="foxyshop_field_control">
		<label for="_salestartdate"><?php _e('Start Date', 'foxyshop'); ?></label>
		<input type="text" id="_salestartdate" name="_salestartdate" value="<?php echo esc_attr($_salestartdate); ?>" style="width: 87px; float: left;" />
		<span style="float: left; margin: 9px 0 0 5px;">mm/dd/yyy</span>
	</div>
	<div class="foxyshop_field_control">
		<label for="_saleenddate"><?php _e('End Date', 'foxyshop'); ?></label>
		<input type="text" id="_saleenddate" name="_saleenddate" value="<?php echo esc_attr($_saleenddate); ?>" style="width: 87px; float: left;" />
		<span style="float: left; margin: 9px 0 0 5px;">mm/dd/yyy</span>
	</div>
	<div style="clear: both;"></div>


	<h4><?php echo _x('Discounts', 'Discounted Prices Applied to Products', 'foxyshop'); ?> <a href="http://wiki.foxycart.com/v/<?php echo esc_attr($foxyshop_settings['version']); ?>/coupons_and_discounts" target="_blank">(<?php _e('reference', 'foxyshop'); ?>)</a></h4>
	<div class="foxyshop_field_control">
	...
	<div id="discount_container">
		...
		<input type="text" name="computed_discount" id="computed_discount" value="<?php echo esc_attr($computed_discount); ?>" />
		<div style="clear:both;"></div>
	...
	<ul id="inventory_levels">
		...
		foreach ($inventory_levels as $ivcode => $iv) {
			if ($ivcode) {
				echo '<li>';
				echo '<input type="text" id="inventory_code_' . esc_attr($i) . '" name="inventory_code_' . esc_attr($i) . '" value="' . esc_attr($ivcode) . '" class="inventory_code" rel="' . esc_attr($i) . '" style="width: 142px;" />';
				echo '<input type="text" id="inventory_count_' . esc_attr($i) . '" name="inventory_count_' . esc_attr($i) . '" value="' . esc_attr($iv['count']) . '" class="inventory_count" rel="' . esc_attr($i) . '" />';
				echo '<input type="text" id="inventory_alert_' . esc_attr($i) . '" name="inventory_alert_' . esc_attr($i) . '" value="' . esc_attr($iv['alert']) . '" class="inventory_count" rel="' . esc_attr($i) . '" />';
				echo "</li>\n";
				$i++;
			}
		}
		?>
		<li><input type="text" id="inventory_code_<?php echo esc_attr($i); ?>" name="inventory_code_<?php echo esc_attr($i); ?>" value="" class="inventory_code" rel="<?php echo esc_attr($i); ?>" style="width: 142px;" /><input type="text" id="inventory_count_<?php echo esc_attr($i); ?>" name="inventory_count_<?php echo esc_attr($i); ?>" value="" class="inventory_count" rel="<?php echo esc_attr($i); ?>" /><input type="text" id="inventory_alert_<?php echo esc_attr($i); ?>" name="inventory_alert_<?php echo esc_attr($i); ?>" value="" class="inventory_count" rel="<?php echo esc_attr($i); ?>" /></li>
	</ul>
	<input type="hidden" name="max_inventory_count" id="max_inventory_count" value="<?php echo esc_attr($i); ?>" />
	<div style="clear:both;"></div>
	<?php } ?>


	<?php if ($foxyshop_settings['enable_subscriptions']) { ?>
	<h4 style="margin-bottom: 3px;"><?php _e('Subscription Attributes', 'foxyshop'); ?> <a href="http://wiki.foxycart.com/v/<?php echo esc_attr($foxyshop_settings['version']); ?>/cheat_sheet#subscription_product_options" target="_blank">(<?php _e('reference'); ?>)</a></h4>
	...
	<div id="foxyshop_subscription_attributes">
		<div class="foxyshop_field_control">
			...
			<input type="text" name="_sub_frequency" id="_sub_frequency" value="<?php echo esc_attr($_sub_frequency); ?>" />
			<span>60d, 2w, 1m, 1y, .5m</span>
		</div>
		<div class="foxyshop_field_control">
			<label for="_sub_startdate"><?php _e('Start Date', 'foxyshop'); ?></label>
			<input type="text" id="_sub_startdate" name="_sub_startdate" value="<?php echo esc_attr($_sub_startdate); ?>" />
			<span>YYYYMMDD or D</span>
		</div>
		<div class="foxyshop_field_control">
			<label for="_sub_enddate"><?php _e('End Date', 'foxyshop'); ?></label>
			<input type="text" id="_sub_enddate" name="_sub_enddate" value="<?php echo esc_attr($_sub_enddate); ?>" />
			<span>YYYYMMDD or D</span>
		</div>
		<div style="clear: both;"></div>
	</div>
	...
	<div class="foxyshop_field_control" id="product_coupon_entry_field"<?php if (!$_coupon) echo ' style="display: none;"'; ?>>
		...
		<input type="text" name="_coupon" id="_coupon" value="<?php echo esc_attr($_coupon); ?>" />
	</div>
	<?php if (version_compare($foxyshop_settings['version'], '2.0', ">=")) { ?>
	<div class="foxyshop_field_control">
		<label for="_expires"><?php _e('Expires', 'foxyshop'); ?></label>
		<input type="text" name="_expires" id="_expires" value="<?php echo esc_attr($_expires); ?>" />
	</div>
	<?php } ?>
	<div style="clear: both;"></div>
	<?php
...
function foxyshop_product_images_setup() {
	...
	if (array_key_exists('error', $upload_dir)) {
		if ($upload_dir['error'] != '') {
			...
			echo '<p>' . wp_kses_post($upload_dir['error']) . '</p>';
			return;
		}
	}
	...
	for ($i=1;$i<=$max_variations;$i++) {
		...
		<div class="product_variation" rel="<?php echo esc_attr($i); ?>" id="variation<?php echo esc_attr($i); ?>">
			...
			<div class="variation_holder" id="variation_holder_<?php echo esc_attr($i); ?>">
				...
				<?php elseif($_variation_type == "text") : ?>
					...
					<div class="foxyshop_field_control text variationoptions">
						<div class="foxyshop_field_control">
							...
							<input type="text" name="_variation_textsize1_<?php echo esc_attr($i); ?>" id="_variation_textsize1_<?php echo esc_attr($i); ?>" value="<?php if (isset($arrVariationTextSize)) echo esc_attr($arrVariationTextSize[0]); ?>" /> <?php _e('characters'); ?>
						</div>
						<div class="foxyshop_field_control">
							<label for="_variation_textsize2_<?php echo esc_attr($i); ?>"><?php _e('Maximum Chars', 'foxyshop'); ?></label>
							<input type="text" name="_variation_textsize2_<?php echo esc_attr($i); ?>" id="_variation_textsize2_<?php echo esc_attr($i); ?>" value="<?php if (isset($arrVariationTextSize)) echo esc_attr($arrVariationTextSize[1]); ?>" /> <?php _e('characters'); ?>
						</div>
						<div style="clear: both;"></div>
					</div>
				...
				<?php elseif($_variation_type == "textarea") : ?>
					...
					<div class="foxyshop_field_control textarea variationoptions">
						...
						<input type="text" name="_variation_textareasize_<?php echo esc_attr($i); ?>" id="_variation_textareasize_<?php echo esc_attr($i); ?>" value="<?php echo esc_attr($_variationValue); ?>" /> (default is 3)
					</div>
			...
			<div class="foxyshop_field_control dkeycontainer"<?php echo wp_kses($dkeyhide, []); ?>>
				...
				<!-- Required -->
				<div class="variation_required_container" rel="<?php echo esc_attr($i); ?>"<?php echo ($_variation_type == 'dropdown' || $_variation_type == 'text' || $_variation_type == 'textarea' || $_variation_type == 'upload' || $_variation_type == 'checkbox' ? '' : ' style="display: none;"'); ?>>
...
function foxyshop_inline_tools_page_js($var_type_array, $variation_key, $saved_variations) {
...
add_action( 'admin_print_footer_scripts', function() use ($var_type_array, $variation_key, $saved_variations) { foxyshop_inline_tools_page_js($var_type_array, $variation_key, $saved_variations); } );
...
function foxyshop_product_meta_save($post_id) {
	...
	if (!wp_verify_nonce((isset($_POST['products_meta_noncename']) ? sanitize_text_field($_POST['products_meta_noncename']) : ""),__FILE__)) return $post_id;
	...
	if (!isset($_POST['_weight1']) && !isset($_POST['_weight2'])) {
		...
		$_weight = (int)sanitize_text_field($_POST['_weight1']) . ' ' . (double)sanitize_text_field($_POST['_weight2']);
	}

	//Remove Illegal Characters From Code
	$_code = trim(sanitize_text_field($_POST['_code']));
	$_code = str_replace('"', '', $_code);
	$_code = str_replace('&', '', $_code);

	//Save Product Detail Data
	foxyshop_save_meta_data('_weight',$_weight);
	foxyshop_save_meta_data('_price',number_format((double)str_replace(",","",sanitize_text_field($_POST['_price'])),FOXYSHOP_DECIMAL_PLACES,".",""));
	foxyshop_save_meta_data('_code',$_code);
	if (isset($_POST['_category'])) foxyshop_save_meta_data('_category',sanitize_text_field($_POST['_category']));
	...
	foxyshop_save_meta_data('_quantity_hide',(isset($_POST['_quantity_hide']) ? sanitize_text_field($_POST['_quantity_hide']) : ""));

	//Require SSO
	if ($foxyshop_settings['enable_sso'] && $foxyshop_settings['sso_account_required'] == 2) {
		foxyshop_save_meta_data('_require_sso',(isset($_POST['_require_sso']) ? sanitize_text_field($_POST['_require_sso']) : ""));
	}

	//Save Sale Pricing Data
	$saleprice = number_format((double)str_replace(",","",sanitize_text_field($_POST['_saleprice'])),FOXYSHOP_DECIMAL_PLACES,".","");
	if ($saleprice == 0) $saleprice = "";
	foxyshop_save_meta_data('_saleprice',$saleprice);
	if (($_salestartdate = strtotime(sanitize_text_field($_POST['_salestartdate']))) === false) foxyshop_save_meta_data('_salestartdate',"999999999999999999");
	else foxyshop_save_meta_data('_salestartdate',$_salestartdate);
	if (($_saleenddate = strtotime(sanitize_text_field($_POST['_saleenddate']))) === false) foxyshop_save_meta_data('_saleenddate',"999999999999999999");
	...
	foreach ($discount_array as $val) {
		if ($_POST['discount_method'] == $val) {
			foxyshop_save_meta_data('_' . $val, sanitize_text_field($_POST['computed_discount']));
		} else {
	...
	if (isset($_POST['_sub_frequency'])) {
		if ($_POST['_sub_frequency'] == "") {
			...
			foxyshop_save_meta_data('_sub_frequency', sanitize_text_field($_POST['_sub_frequency']));
		}
		if ($_POST['_sub_startdate'] == "") {
			foxyshop_save_meta_data('_sub_startdate', "");
		} else {
			foxyshop_save_meta_data('_sub_startdate', sanitize_text_field($_POST['_sub_startdate']));
		}
		if ($_POST['_sub_enddate'] == "") {
			foxyshop_save_meta_data('_sub_enddate', "");
		} else {
			foxyshop_save_meta_data('_sub_enddate', sanitize_text_field($_POST['_sub_enddate']));
		}
	}

	//Extra Options
	if (isset($_POST['do_coupon'])) {
		foxyshop_save_meta_data('_coupon', sanitize_text_field($_POST['_coupon']));
	} else {
		foxyshop_save_meta_data('_coupon', '');
	}
	if (isset($_POST['_empty'])) {
		foxyshop_save_meta_data('_empty', sanitize_text_field($_POST['_empty']));
	} else {
		foxyshop_save_meta_data('_empty', '');
	}

	if (isset($_POST['_cart'])) {
		foxyshop_save_meta_data('_cart', sanitize_text_field($_POST['_cart']));
	} else {
		foxyshop_save_meta_data('_cart', '');
	}

	if (isset($_POST['_expires'])) {
		foxyshop_save_meta_data('_expires', sanitize_text_field($_POST['_expires']));
	} else {
	...
	if (isset($_POST['_related_products_list'])) {
		foxyshop_save_meta_data('_related_products', sanitize_text_field(implode(",",$_POST['_related_products_list'])));
	} else {
		foxyshop_save_meta_data('_related_products', "");
	}
	if (isset($_POST['_related_order'])) foxyshop_save_meta_data('_related_order', sanitize_text_field($_POST['_related_order']));
	if (isset($_POST['_addon_order'])) foxyshop_save_meta_data('_addon_order', sanitize_text_field($_POST['_addon_order']));

	//Save Bundled Product Data
	if (isset($_POST['_bundled_products_list'])) {
		foxyshop_save_meta_data('_bundled_products', sanitize_text_field(implode(",",$_POST['_bundled_products_list'])));
	} else {
	...
	if (isset($_POST['_addon_products_list'])) {
		foxyshop_save_meta_data('_addon_products', sanitize_text_field(implode(",",$_POST['_addon_products_list'])));
	} else {
	...
	if ($foxyshop_settings['manage_inventory_levels']) {
		...
		for ($i=1; $i<=(int)sanitize_text_field($_POST['max_inventory_count']); $i++) {
			if ($_POST['inventory_code_'.$i] && $_POST['inventory_count_'.$i] != '') {
				$alert_level = sanitize_text_field($_POST['inventory_alert_'.$i]);
				if ($alert_level != '') $alert_level = (int)$alert_level;
				$inventory_array[stripslashes(str_replace("'","",sanitize_text_field($_POST['inventory_code_'.$i])))] = array("count" => (int)sanitize_text_field($_POST['inventory_count_'.$i]), "alert" => $alert_level);
			}
		}
	...
	$currentID = 1;
	$variations = array();
	$max_variations = (int)sanitize_text_field($_POST['max_variations']);
	for ($i=1;$i<=$max_variations;$i++) {

		//Get Target From Sort Numbers
		$target_id = 0;
		for ($k=1;$k<=$max_variations;$k++) {
		...
		$_variationName = trim(str_replace("&","and",str_replace(".","",str_replace('"','',sanitize_text_field($_POST['_variation_name_'.$target_id])))));
		...
		if ($_variationType == 'text') {
			$_variationValue = sanitize_text_field($_POST['_variation_textsize1_'.$target_id])."|".sanitize_text_field($_POST['_variation_textsize2_'.$target_id]);
			...
			$_variationValue = sanitize_text_field($_POST['_variation_checkbox_'.$target_id]);
			...
			$_variationValue = sanitize_text_field($_POST['_variation_hiddenfield_'.$target_id]);
		}
	...
	if ($foxyshop_settings['google_product_support']) {
		foreach($google_product_field_names as $field) {
			foxyshop_save_meta_data("_" . $field, sanitize_text_field($_POST["_" . $field]));
		}
	}
